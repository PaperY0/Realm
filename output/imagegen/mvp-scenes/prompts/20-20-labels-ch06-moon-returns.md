USE CASE
Create one final 2048 x 1152 PNG reference image for the Dreambook Realm MVP. Rendering mode: storybook narrative. The image guides a shallow-layer frontend scene and must contain no text or interface.
...
Scene: same drawer and bridge language; moonlight emerges only from the drawer seam and touches one blank label, transforming it into a black-paper-label kite
Main action: Miannian shields then recognizes the returning ornament light
Single symbolic focus: one local moonlight and one transforming kite
Show one main action, one restrained emotion and one symbolic event only.

COMPOSITION AND FRONTEND SAFE AREA
Canvas is 16:9 landscape. Keep all essential subjects outside the outer 5% crop zone. Character height is normally 20% to 35% of frame height. Required frontend-safe region: right-side 35% low-detail darkened alley wall. Build this region naturally from sky, mist, water, warm page air, grass or a quiet wall, never a blank UI rectangle. Magic occupies no more than roughly 10% to 15% of the image.

CHARACTER IDENTITY LOCKS
Characters: Miannian.
Original warm-white lamb with a compact rounded body, short legs, restrained dot eyes, small asymmetric curled horns and one fixed moon ornament beside the left horn. Never blue; do not copy the reference-book sheep face, proportions, clothing or composition.
