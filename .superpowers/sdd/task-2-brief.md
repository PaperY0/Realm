...
Read this brief as the exact requirements for Task 2.

Files:
- Modify only: `C:/Users/wsy19/Desktop/Realm/src/styles.css`

Implement the following expression-page-scoped CSS. Append it in the existing Stage 18 expression styles so later rules win, and do not change HTML or JavaScript.
...
Run `node tests/stage18-expression-paper.test.js`, `node --check src/app.js`, and `git diff --check`. The static test must pass. Commit only the CSS change. Write `.superpowers/sdd/task-2-report.md` with commit hash, commands/outputs, and concerns. Return only status, commit, one-line test summary, and concerns.
