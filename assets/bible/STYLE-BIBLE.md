...
- 所有现行图像提示词必须包含本文件第 19 节的 V3.3 核心块和统一负面块。
- 画风统一不等于构图重复；角色、地点、季节和故事可以变化，但核心视觉规则不得变化。
- 本项目不再保留或认可任何旧版、实验分支或失败轮次画风文档。
- [`STYLE-EVALUATION-V3.3.md`](STYLE-EVALUATION-V3.3.md) 只负责评分，[`STYLE-VALIDATION-V3.3.md`](STYLE-VALIDATION-V3.3.md) 只记录最终验收；两者都不得覆盖本文件。
