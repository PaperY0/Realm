USE CASE
Create one final 2048 x 1152 PNG reference image for the Dreambook Realm MVP. Rendering mode: explorable world. The image guides a shallow-layer frontend scene and must contain no text or interface.

REFERENCE ROLES
- scene 02
- world guide anchor
Treat identity references as identity-only and style references as medium/color/line-only. Never copy their composition or any visible writing.

SCENE AND MAIN ACTION
Scene: view through a small amount of foreground doorframe onto the same path narrowing gently into the world
Main action: world guide walks ahead with the green satchel visible from behind
Single symbolic focus: threshold and winding road
Show one main action, one restrained emotion and one symbolic event only.

COMPOSITION AND FRONTEND SAFE AREA
Canvas is 16:9 landscape. Keep all essential subjects outside the outer 5% crop zone. Character height is normally 20% to 35% of frame height. Required frontend-safe region: upper-right low-detail sky and distant slope. Build this region naturally from sky, mist, water, warm page air, grass or a quiet wall, never a blank UI rectangle. Magic occupies no more than roughly 10% to 15% of the image.

CHARACTER IDENTITY LOCKS
Characters: world guide.
Preserve the cream rounded body, tiny dot eyes, very small mouth, green diagonal strap and fixed small green satchel. No ears, wings, clothing redesign or animal reinterpretation.
Keep silhouette, proportions, face placement, palette and every fixed prop stable across adjacent scenes.

OUTPUT INTENT
Original adult fairy-tale watercolor picture-book illustration, full composite scene, no transparent layers. Deliver PNG at 2048 x 1152. No text, pseudo-text, letters, numbers, buttons, logo, signature, account name or watermark anywhere. Do not draw a UI, form, chat panel, game map or game portal.
...
WORLD GUIDE IDENTITY OVERRIDE — USER-PROVIDED FOUR-VIEW IMAGE IS THE HIGHEST AUTHORITY
The second input is the exact world-guide IP turnaround. Replace every incorrect rounded-head guide in the first input with this exact character while preserving the entire scene, environment, composition, lighting and all non-character objects.
Identity locks: cream rounded pear-shaped body; TWO fixed tall triangular points on top with a shallow central notch; tiny matte oval black eyes; extremely small horizontal mouth; short U-shaped line arms; two tiny feet; olive-green diagonal strap; fixed rounded olive-green satchel with flap and one cream clasp. Preserve the same body proportions and continuous warm-brown outline. Do not round away the two top points, do not add whiskers, tail, clothing, fingers, glossy eyes or blush. Choose the matching front, three-quarter, side or rear orientation from the turnaround according to the scene action. Keep the guide at the original scene scale and location unless the base scene instruction explicitly requires movement.
