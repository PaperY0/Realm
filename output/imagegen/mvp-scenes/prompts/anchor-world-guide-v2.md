USE CASE
Create one final 2048 x 1152 PNG reference image for the Dreambook Realm MVP. Rendering mode: identity model sheet. The image guides a shallow-layer frontend scene and must contain no text or interface.

REFERENCE ROLES
- assets/concepts/references/world-guide-source.png
- assets/concepts/references/e929278a43934a5764b51f0c833b8e97.png
...
Scene: single clean identity anchor, front three-quarter view
Main action: quietly waiting with a restrained expression
Single symbolic focus: fixed green cross-body strap and small green satchel
Show one main action, one restrained emotion and one symbolic event only.

COMPOSITION AND FRONTEND SAFE AREA
Canvas is 16:9 landscape. Keep all essential subjects outside the outer 5% crop zone. Character height is normally 20% to 35% of frame height. Required frontend-safe region: warm-white uncluttered background around the full figure. Build this region naturally from sky, mist, water, warm page air, grass or a quiet wall, never a blank UI rectangle. Magic occupies no more than roughly 10% to 15% of the image.
...
REVISION — CHANGE ONLY THESE THREE DEVIATIONS
1. Remove both pointed ear-like peaks completely. The top of the cream body must become one smooth, softly rounded seed-like dome with no ears, horns, antennae or animal reading.
2. Replace the entire river, tree, flower and landscape background with a quiet warm-white watercolor page field containing only a very faint soft floor wash beneath the feet.
3. Keep the character contour clean, continuous and clearly separated from the low-detail background. Preserve the exact green diagonal strap, small green satchel, body proportions, dot eyes, tiny mouth and restrained expression from the first input.
Do not redesign anything else. Show one full-body front three-quarter identity view centered with generous empty space.
